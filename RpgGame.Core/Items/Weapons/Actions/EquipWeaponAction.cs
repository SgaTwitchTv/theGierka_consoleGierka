...
{
    // DESIGN PATTERN: Command - encapsulates the equip operation as an invokable action.
    public sealed class EquipWeaponAction : InventoryActionBase // EquipWeaponAction is a specific type of InventoryActionBase that handles equipping weapons for a player. It takes in the player, the weapon to be equipped, and the hand slot (left or right) as parameters. When executed, it calls the Equip method on the weapon's HandRequirement, passing in the player, weapon, and slot to perform the equipping action.
    {
        ...
        private readonly Weapons.IWeapon _weapon;
        private readonly HandSlot _slot;

        public EquipWeaponAction(Player player, Weapons.IWeapon weapon, HandSlot slot): base(slot == HandSlot.Left ? "Equip Left" : "Equip Right")   // The constructor initializes the EquipWeaponAction with the player, weapon, and hand slot. It also sets the action name based on the hand slot (either "Equip Left" or "Equip Right").
        {
            _player = player;
            _weapon = weapon;
            _slot = slot;
